...
def evaluate(opt, net, eval_range, prediction_txt_path, reference):
    ...
    result = {}
    for i, (frames, depth_feat, video_ids) in tqdm(enumerate(eval_loader)):

        frames, depth_feat = frames.to(DEVICE), depth_feat.to(DEVICE)
        outputs, _ = net(frames, depth_feat, None)
